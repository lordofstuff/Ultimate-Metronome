...
public class MyExampleView extends View {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		final TypedArray a = getContext().obtainStyledAttributes(attrs,
				R.styleable.MyExampleView, defStyle, 0);

		mExampleString = a.getString(R.styleable.MyExampleView_exampleString);
		mExampleColor = a.getColor(R.styleable.MyExampleView_exampleColor,
				mExampleColor);
		// Use getDimensionPixelSize or getDimensionPixelOffset when dealing
		// with
		// values that should fall on pixel boundaries.
		mExampleDimension = a.getDimension(
				R.styleable.MyExampleView_exampleDimension, mExampleDimension);

		if (a.hasValue(R.styleable.MyExampleView_exampleDrawable)) {
			mExampleDrawable = a
					.getDrawable(R.styleable.MyExampleView_exampleDrawable);
